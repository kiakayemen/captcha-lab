...
5. runs pretrained PARSeq-tiny over each transformed variant;
...
- `ocr.py`: PARSeq-tiny reader setup
...
Pretrained PARSeq-tiny is run across the preprocessing variants.
...
subtype is checked even when an earlier subtype has an appointment. Appointment
results notify the admins, and the final run status is reported only after all
configured subtypes have completed. Technical failures retry the incomplete
subtype with another fresh browser, account, and proxy.
